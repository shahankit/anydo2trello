...
class CreateOnTrello(object):
	...
	def create_list(self, list_name, trello_board):
		...
		"""
		lists_list = trello_board.get_lists()
		for board_list in lists_list:
			if board_list.name == list_name:
				return board_list
		return trello_board.add_list({'name':list_name})

	def create_card(self, card_title, trello_list, desc):
		"""
	...
	def create_checklist(self, trello_card, checklist_title='Checklist'):
		"""
		Adds a checklist to trello_card in parameters. Returns a new
		trello_checklist object.
		"""
		return trello_card.add_checklists({'name': checklist_title})

	def create_checkitem(self, trello_checklist, checkitem_name, checked=0):
		"""
		Adds a new checkitem to trello_checklist in parameters.
		"""
		return trello_checklist.add_item({'name':checkitem_name, 'checked':checked})
